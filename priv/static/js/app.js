...
Elm.NoteSaver = Elm.NoteSaver || {};
Elm.NoteSaver.make = function (_elm) {
   "use strict";
   _elm.NoteSaver = _elm.NoteSaver || {};
   if (_elm.NoteSaver.values)
   return _elm.NoteSaver.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "NoteSaver",
   $Basics = Elm.Basics.make(_elm),
   ...
   model) {
      return function () {
         ...
         {case "Delete":
            return function () {
                 var remainingNotes = A2($List.filter,
                 function (n) {
                    return !_U.eq(n.id,
                    action._0);
                 },
   ...
   var noteItem = F2(function (address,
   note) {
      return A2($Html.li,
      _L.fromArray([$Html$Attributes.$class("note_memo")]),
      _L.fromArray([A2($Html.span,
                   _L.fromArray([$Html$Attributes.$class("memo")]),
                   _L.fromArray([$Html.text(note.memo)]))
                   ,A2($Html.button,
   ...
   var SetNotes = function (a) {
      return {ctor: "SetNotes"
             ,_0: a};
   };
   var NoOp = {ctor: "NoOp"};
   var noteList = Elm.Native.Port.make(_elm).inboundSignal("noteList",
   "NoteSaver.NoteList",
   function (v) {
      return typeof v === "object" && v instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.map(function (v) {
         return typeof v === "object" && "id" in v && "donor_id" in v && "memo" in v ? {_: {}
                                                                                       ,id: typeof v.id === "number" ? v.id : _U.badPort("a number",
                                                                                       v.id)
                                                                                       ,donor_id: typeof v.donor_id === "number" ? v.donor_id : _U.badPort("a number",
                                                                                       v.donor_id)
                                                                                       ,memo: typeof v.memo === "string" || typeof v.memo === "object" && v.memo instanceof String ? v.memo : _U.badPort("a string",
                                                                                       v.memo)} : _U.badPort("an object with fields `id`, `donor_id`, `memo`",
         v);
      })) : _U.badPort("an array",v);
   });
   var incomingActions = A2($Signal.map,
   SetNotes,
   noteList);
   var newNote = F3(function (id,
   donor_id,
   memo) {
      return {_: {}
             ,donor_id: donor_id
   ...
   var app = $StartApp.start({_: {}
                             ,init: {ctor: "_Tuple2"
                                    ,_0: initialModel
                                    ,_1: $Effects.none}
                             ,inputs: _L.fromArray([incomingActions])
                             ,update: update
                             ,view: view});
   var main = app.html;
   _elm.NoteSaver.values = {_op: _op
                           ,app: app
                           ,main: main
                           ,Note: Note
                           ,Model: Model
...
socket.connect();
var donor = document.getElementById('donor_data'),
...
channel.join().receive("ok", function (notes) {
  return elmApp.ports.noteList.send(notes);
})
// "ok", resp => console.log("CHANNEL JOINED", resp))
.receive("error", function (resp) {
  return console.log(resp);
});
channel.on("new:notes", function (notes) {
  return console.log("CHANNEL ON", notes);
});
});
