...
Elm.Saints.Note.make = function (_elm) {
   ...
   var makeModel = function (note) {
      return {note: note,editing: false,$new: false};
   };
   var Model = F3(function (a,b,c) {
      return {note: a,editing: b,$new: c};
   });
   var initNote = {id: -1
                  ,donor_id: -1
                  ,author: ""
                  ,memo: ""
                  ,updated_at: ""};
   var init = {note: initNote,editing: false,$new: false};
   var newNote = function (donor_id) {
      var note = initNote;
      var newNote = _U.update(note,{donor_id: donor_id});
      return {note: newNote,editing: true,$new: true};
   };
...
Elm.Saints.Donor.make = function (_elm) {
   ...
   var fullNameText = function (d) {
      ...
      "(",
      A2($Basics._op["++"],$Basics.toString(d.id),")"));
      return $Html.text(A2($String.join,
      " ",
      _U.list([d.title
              ,d.firstName
   ...
   var labelSave = function (model) {
      return _U.cmp(model.donor.id,
      0) < 0 ? $Html.text("save new donor") : $Html.text("change name");
   };
   ...
   var updatedDonorDonations = F3(function (donations,id,action) {
      ...
      var newDonations = _U.eq($Basics.toString(action),
      "Delete") ? A2($List.filter,
      function (d) {
         return !_U.eq(d.donation.id,id);
      },
      donations) : A2($List.map,donationAction,donations);
      return newDonations;
   });
   var updatedDonorAddresses = F3(function (addresses,id,action) {
      ...
      var newAddresses = _U.eq($Basics.toString(action),
      "Delete") ? A2($List.filter,
      function (d) {
         return !_U.eq(d.address.id,id);
      },
      addresses) : A2($List.map,addrAction,addresses);
      return newAddresses;
   });
   var updatedDonorPhones = F3(function (phones,id,action) {
      ...
      var newPhones = _U.eq($Basics.toString(action),
      "Delete") ? A2($List.filter,
      function (d) {
         return !_U.eq(d.phone.id,id);
      },
      phones) : A2($List.map,phoneAction,phones);
      return newPhones;
   });
   var updatedDonorNotes = F3(function (notes,id,action) {
      ...
      var newNotes = _U.eq($Basics.toString(action),
      "Delete") ? A2($List.filter,
      function (d) {
         return !_U.eq(d.note.id,id);
      },
      notes) : A2($List.map,noteAction,notes);
      return newNotes;
   });
   var update = F2(function (action,model) {
      var _p1 = action;
   ...
   var Aka = function (a) {    return {ctor: "Aka",_0: a};};
   ...
   var inputAka = F2(function (address,model) {
      var donor = model.donor;
      return A2($Html.input,
      _U.list([$Html$Attributes.id("aka")
              ,$Html$Attributes.type$("text")
              ,$Html$Attributes.placeholder("Nick name")
              ,$Html$Attributes.autofocus(true)
              ,$Html$Attributes.name("aka")
              ,A3($Html$Events.on,
              "input",
              $Html$Events.targetValue,
              function (str) {
   ...
   var cancelSave = F2(function (address,model) {
      var cancel = A2($Html.button,
      _U.list([A2($Html$Events.onClick,
      detailsGet.address,
      model.donor)]),
      _U.list([$Html.text("reset")]));
      var save = A2($Html.button,
      _U.list([A2($Html$Events.onClick,
      donorUpdate.address,
      model.donor)]),
      _U.list([labelSave(model)]));
      var buttons = _U.cmp(model.donor.id,
      0) < 0 ? _U.list([save]) : _U.list([save,cancel]);
      return A2($Html.span,_U.list([cancelSaveStyle(model)]),buttons);
   });
   var donorNameEdit = F2(function (address,model) {
      return A2($Html.ul,
      _U.list([editStyle(model)]),
      _U.list([A2($Html.li,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2"
                                               ,_0: "padding-bottom"
                                               ,_1: "1px"}]))]),
   ...
   var view = F2(function (address,model) {
      ...
      donor.phones));
      return A2($Html.li,
      _U.list([A2(onClickDetails,address,model)]),
...
var channel = _socket2["default"].channel("donors:list");
...
elmApp.ports.updateAddress.subscribe(function (address) {
  if (address.id < 0) {
    channel.push("create_address", address);
  } else {
...
elmApp.ports.updatePhone.subscribe(function (phone) {
  if (phone.id < 0) {
    channel.push("create_phone", phone);
  } else {
